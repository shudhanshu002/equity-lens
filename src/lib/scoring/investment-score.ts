...
function scoreBalanceSheet(financials: FinancialSnapshot): number {
  const debtToEquity = financials.debtToEquity ?? 1;
  const currentRatio = financials.currentRatio ?? 1;
  const fcfBonus = financials.freeCashFlowPositive ? 20 : 0;
  ...
  return clamp(debtScore + liquidityScore + fcfBonus);
}
